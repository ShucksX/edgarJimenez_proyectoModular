...
public class ResLaberinto {
	private Scene resLaberintoScene;
	...
	public ResLaberinto(Scene returnScene, Stage primaryStage, boolean mode, String userID) {
		this.userID = userID;
		this.mode = mode;
		...
		Label lblTitulo = new Label ("Resultados: Laberintos");
		...
		//Set Scene
		resLaberintoScene = new Scene(layout,Utilities.windowWidth,Utilities.windowHeight);
		resLaberintoScene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
	}
	...
	public void fillTableId(TableView<LSData> table) throws IOException, InterruptedException {
		HttpClient client = HttpClient.newHttpClient();
		HttpRequest request = HttpRequest.newBuilder(URI.create(
				Utilities.getBaseURL() + "/Resultado/obtenerlaberintos.php?idusuario=" + userID)).GET().build();
		HttpResponse<String> response = client.send(request, BodyHandlers.ofString());
		setupTable(table,response.body());
	}
	...
	public void buscar(TableView<LSData> table) throws IOException, InterruptedException {
		String url = Utilities.getBaseURL() + "/Resultado/obtenerlaberintos.php";
	...
	private void ventanaGrafica(TableView<LSData> table) {
		...
		window.setTitle("Grafica de resultados: Laberintos");
		...
		label.setText("Grafica de resultados: Laberintos");
		label.setFont(fontTitulo);
		
		final NumberAxis xAxis  = new NumberAxis();
		final NumberAxis yAxis  = new NumberAxis();
		xAxis.setLabel("Dias");
		yAxis.setLabel("Tiempo tardado");
		
		final LineChart<Number,Number> lineChart = 
	                new LineChart<Number,Number>(xAxis,yAxis);
		
		lineChart.setTitle("Grafica de resultados: Laberintos");
        //defining a series
        ...
        if (series.getData().size()<=2) {
        	textSLR = "Le recomendamos hacer más test antes de realizar conclusiones";
        }
        else {
	        if (slr.getBeta1() < 0) {
        ...
        sucursal.setText("Si considera necesario asistir a un centro de atención:");
        
        Label sucursal2 = new Label();
        sucursal2.setFont(fontTexto);
        sucursal2.setText("Le invitamos a visitar la ventana de sucursales en la selección de test");
