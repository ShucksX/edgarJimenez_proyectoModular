...
public class ResSiluetas {
	private Scene resSiluetasScene;
	...
	public ResSiluetas(Scene returnScene, Stage primaryStage, boolean mode, String userID) {
		this.userID = userID;
		this.mode = mode;
		...
		Label lblTitulo = new Label ("Resultados: Siluetas");
		...
		TableColumn<LSData, String> puntos = new TableColumn <> ("Aciertos sobre 10");
		...
		Label txtPuntos= new Label ("Aciertos sobre 10:");
		txtPuntos.setFont(fontTexto);
		fieldPuntos = new TextField();
		fieldPuntos.setPromptText("Aciertos sobre 10");
		...
		//Set Scene
		resSiluetasScene = new Scene(layout,Utilities.windowWidth,Utilities.windowHeight);
		resSiluetasScene.getStylesheets().add(getClass().getResource("application.css").toExternalForm());
	}
	...
	public void fillTableId(TableView<LSData> table) throws IOException, InterruptedException {
		HttpClient client = HttpClient.newHttpClient();
		HttpRequest request = HttpRequest.newBuilder(URI.create(
				Utilities.getBaseURL() + "/Resultado/obtenersiluetas.php?idusuario=" + userID)).GET().build();
		HttpResponse<String> response = client.send(request, BodyHandlers.ofString());
		setupTable(table,response.body());
	}
	...
	public void buscar(TableView<LSData> table) throws IOException, InterruptedException {
		String url = Utilities.getBaseURL() + "/Resultado/obtenersiluetas.php";
	...
	private void ventanaGrafica(TableView<LSData> table) {
		...
		window.setTitle("Grafica de resultados: Siluetas");
		...
		label.setText("Grafica de resultados: Siluetas");
		...
		lineChart.setTitle("Grafica de resultados: Siluetas");
        //defining a series
